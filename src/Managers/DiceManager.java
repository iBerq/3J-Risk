    ...
    void rollDice(int attackerRange) {
        ...
        for (int i = 0; i < numOfAttackerDice; i++) {
            ...
            if (i == 0) {
                ...
                if (temp >= attackersScores.get(0)) {
                    attackersScores.add(attackersScores.get(0));
                    attackersScores.set(0, temp);
                } else if (temp < attackersScores.get(0)) {
                    attackersScores.add(i, temp);
                }
            } else {
                if (i == 2 && temp >= attackersScores.get(0)) {
                    attackersScores.add(attackersScores.get(i - 1));
                    attackersScores.set(i - 1, attackersScores.get(0));
                    attackersScores.set(0, temp);
                    break;
